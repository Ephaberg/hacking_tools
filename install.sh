...
echo -e "${BLUE}                                    https://github.com/Ephaberg/hacking_tools ${NC}"
...
install_dir="/usr/share/hacking_tools"
bin_dir="/usr/bin"
...
if [[ $choice =~ ^[1-2]+$ ]]; then
    ...
    if curl -s -m 10 https://www.google.com > /dev/null || curl -s -m 10 https://www.github.com > /dev/null; then
        ...
        echo "";
        echo -e "${YELLOW}[✔] Downloading hackingtools...${NC}"
        if sudo git clone https://github.com/Ephaberg/hacking_tools.git $install_dir; then
...
#            echo '#!/bin/bash' > hackingtools.sh
            echo '#!/bin/bash' > $install_dir/hackingtools.sh
            echo "source $install_dir/venv/bin/activate" >> $install_dir/hackingtools.sh
            echo "python3 $install_dir/hackingtools.py \$@" >> $install_dir/hackingtools.sh
            chmod +x $install_dir/hackingtools.sh
            sudo mv $install_dir/hackingtools.sh $bin_dir/hackingtools
            echo -e "${GREEN}[✔] Script created successfully [✔]"
        else
            echo -e "${RED}[✘] Failed to download hackingtools [✘]"
            exit 1
        fi
    ...
    if [ -d $install_dir ]; then
        ...
        echo -e  "${ORANGE}[+]+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++[+]"
        echo     "[+]                                                             [+]"
        echo -e  "${ORANGE}[+]     ✔✔✔ Now Just Type In Terminal (hacking_tools) ✔✔✔      [+]"
        echo     "[+]                                                             [+]"
        echo -e  "${ORANGE}[+]+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++[+]"
    else
